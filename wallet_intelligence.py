...
# ── Token quality filters — mirrored from SpiderWalletBot2.py ─────────────────
# Both files must agree on the same six constants; centralizing here means
# callers in either module reference the same tuning, and operators only have
# to set the env vars (MIN_VOLUME, MIN_AGE_SECONDS, MAX_AGE_SECONDS, MIN_HOLDERS,
# REQUIRE_RENOUNCED, MIN_LIQUIDITY_RATIO) once per deployment.
...
def reconcile_seed_roster(wallets: list) -> int:
    """
    ...
    it was removed from FALLBACK_WALLETS in code. Without this, editing that
    ...
    with _DB_LOCK:
        rows = conn.execute(
            "SELECT wallet FROM active_roster WHERE status='active' AND source='seed'").fetchall()
        stale = [r[0] for r in rows if r[0] not in current]
        for w in stale:
            conn.execute(
                "UPDATE active_roster SET status='retired' WHERE wallet=?", (w,))
        if stale:
            conn.commit()
    if stale:
        logger.info("🪦 Retired %d wallet(s) removed from FALLBACK_WALLETS: %s",
                   len(stale), ", ".join(w[:8]+"…" for w in stale))
    return len(stale)
        ...
        for w in set(wallets):
            try:
                conn.execute(
                    ...
                    "(wallet, mint, entry_mcap, sol_spent, alert_ts) VALUES (?,?,?,?,?)",
                    (w, mint, entry_mcap, sol_by_wallet.get(w), now))
            except Exception as e:
...
def record_outcome_event(mint: str, outcome_type: str, roi_pct: float, win: bool):
    ...
    with _DB_LOCK:
        ...
        conn.execute(
            ...
            (mint, resolved_ts, outcome_type, roi_pct, 1 if win else 0))
        if row:
