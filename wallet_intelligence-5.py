"""
wallet_intelligence.py — DualChainWalletBot Intelligence Layer
=============================================================
...
RUG_CONFIRM_STRIKES        = 2      # consecutive risk-monitor passes a rug signal must persist
                                     # across before actually alerting — brand-new tokens can show
                                     # momentarily missing/stale liquidity data from indexing lag,
                                     # not an actual rug; requiring it to still be true ~6 min later
                                     # (2 x RISK_MONITOR_INTERVAL_SECS) filters that out.
...
DB_EXISTED_AT_BOOT = os.path.exists(DB_PATH)   # checked at import time, before _get_conn()
                                                # ever creates the file — the only reliable way
                                                # to tell "genuinely fresh install" apart from
                                                # "ephemeral storage wiped it again". If this is
                                                # False on anything but the very first-ever boot,
                                                # the DB isn't persisting across restarts.
...
def _init_schema(conn):
    ...
    CREATE TABLE IF NOT EXISTS token_lifecycle (
        ...
        rug_signal_streak   INTEGER DEFAULT 0,
    ...
    -- Generic key-value store for small pieces of state that need to survive
    -- restarts but don't warrant their own table — e.g. the registered
    -- Helius webhook ID, so the bot can PUT/update it on restart instead of
    -- POSTing a brand new webhook every time (which costs credits and can
    -- leave orphaned webhooks all independently delivering the same events).
    -- Deliberately NOT chain-scoped with a column — this stays a flat
    -- key-value store, and future per-chain settings (e.g. a BNB Chain
    -- webhook ID alongside the Solana one) just get their own distinct key
    -- name (e.g. "webhook_id_BNB") rather than needing a schema change here.
    ...
    -- One row per (wallet, mint, trade cycle) — the foundation for the
    -- extended wallet quality score. Written at alert time (entry_mcap,
    -- sol_spent), then filled in when the trade resolves (resolved_ts,
    -- roi_pct, win, outcome_type) via record_outcome_event. hold_time is
    -- derived as resolved_ts - alert_ts, not stored directly.
    CREATE TABLE IF NOT EXISTS wallet_trade_log (
        id              INTEGER PRIMARY KEY AUTOINCREMENT,
        wallet          TEXT NOT NULL,
        mint            TEXT NOT NULL,
        entry_mcap      REAL,
        ...
        win             INTEGER,
        chain           TEXT NOT NULL DEFAULT 'SOL',
        UNIQUE(wallet, mint, alert_ts)
    );
    CREATE INDEX IF NOT EXISTS idx_wallet_trade_log_wallet ON wallet_trade_log(wallet);
    """)
    conn.commit()
...
def _migrate_schema(conn):
    ...
    migrations = [
        ...
        ("token_lifecycle", "rug_signal_streak",  "INTEGER DEFAULT 0"),
        ...
        # ── Phase 1 multi-chain readiness: every chain-scoped table gets a
        # 'chain' column, defaulting to 'SOL' so 100% of existing data is
        # correctly and automatically classified as Solana with zero manual
        # backfill needed. Purely additive — no existing query, index, or
        # primary key changes, so current Solana behavior is unaffected.
        # bot_settings is deliberately excluded (stays a flat global
        # key-value store; future per-chain settings just get distinct key
        # names instead, e.g. "webhook_id_BNB").
        ("token_launches",         "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("early_entries",          "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("wallet_early_entry_avg", "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("token_lifecycle",        "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("buy_sequences",          "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("leader_edges",           "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("leader_scores",          "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("wallet_metrics",         "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("wallet_confidence",      "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("tier_history",           "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("active_roster",          "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("discovery_candidates",   "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("legacy_wallet_stats",    "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("daily_wallet_wins",      "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("alert_outcomes",         "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("outcome_log",            "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
        ("wallet_trade_log",       "chain", "TEXT NOT NULL DEFAULT 'SOL'"),
    ]
...
def record_token_launch(mint: str, ts: int, symbol: str = "", chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        conn.execute(
            "INSERT OR IGNORE INTO token_launches (mint, launch_ts, first_symbol, chain) VALUES (?,?,?,?)",
            (mint, ts, symbol, chain))
        conn.commit()
...
def upsert_token_lifecycle(mint: str, symbol: str, price: float, mcap: float, ts: int,
                           liquidity: float = None, price_change_5m: float = None,
                           chain: str = "SOL") -> dict:
    """
    ...
    with _DB_LOCK:
        ...
        if row is None:
            conn.execute(
                ...
                " ath_ts, last_price, last_mcap, last_updated, roi_pct, status, last_liquidity, chain) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,0,'active',?,?)",
                (mint, symbol, price, mcap, ts, price, mcap, ts, price, mcap, ts, liquidity, chain))
            conn.commit()
            row = conn.execute("SELECT * FROM token_lifecycle WHERE mint=?", (mint,)).fetchone()
        ...
        # Debounced: a single noisy/incomplete reading from the price data
        # provider (common for very new tokens still being indexed) shouldn't
        # be enough to fire a "possible rug" alert on its own. The signal has
        # to reappear on a second consecutive check before it actually fires.
        # It's also suppressed outright if TP1/TP2 just hit in this SAME
        # pass — a token that's genuinely rugging doesn't simultaneously show
        # a confirmed price pump, so that combination means the liquidity/
        # price-crash reading was almost certainly the bad data point, not
        # the token.
        rug_alerted       = data["rug_alerted"] or 0
        rug_signal_streak = data["rug_signal_streak"] or 0
        ...
        if not rug_alerted:
            ...
            if (initial_liquidity and liquidity is not None and initial_liquidity > 0):
                ...
                if (liq_ratio <= (1 - RUG_LIQUIDITY_DROP_PCT / 100)
                    ...
                    raw_signals.append("liquidity_removed")
            if price_change_5m is not None and price_change_5m <= -RUG_PRICE_CRASH_PCT:
                raw_signals.append("price_crash")

            contradicted_by_tp = tp1_just_hit or tp2_just_hit
            if raw_signals and not contradicted_by_tp:
                rug_signal_streak += 1
                if rug_signal_streak >= RUG_CONFIRM_STRIKES:
                    rug_signals = raw_signals
                    rug_alerted = 1
            else:
                rug_signal_streak = 0
        ...
        conn.execute(
            ...
            "stop_loss_alerted=?, rug_alerted=?, rug_signal_streak=?, last_liquidity=? WHERE mint=?",
            (symbol or data["symbol"], ath_price, ath_mcap, ath_ts, low_price, low_ts,
             price, mcap, ts,
             hit_2x, hit_5x, hit_10x, hit_2x_ts, hit_5x_ts, hit_10x_ts,
             dump_2x, dump_5x, dump_10x, dump_2x_ts, dump_5x_ts, dump_10x_ts,
             tp1_hit, tp2_hit, tp1_hit_ts, tp2_hit_ts,
             roi_pct, dd_alerted_pct, stop_loss_alerted, rug_alerted, rug_signal_streak,
             last_liquidity, mint))
        conn.commit()
        ...
        data.update({
            ...
            "rug_signal_streak": rug_signal_streak,
...
def set_trade_plan(mint: str, stop_loss_price: float = None, initial_liquidity: float = None,
                   tp1_price: float = None, tp2_price: float = None, chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        ...
        if row is None:
            ...
            conn.execute(
                "INSERT INTO token_lifecycle (mint, status, chain) VALUES (?, 'active', ?)",
                (mint, chain))
        conn.execute(
            ...
            "stop_loss_alerted=0, rug_alerted=0, rug_signal_streak=0, smart_exit_alerted=0 WHERE mint=?",
...
def get_active_lifecycle_mints(max_age_hours: int = LIFECYCLE_ACTIVE_HOURS, chain: str = "SOL") -> list:
    """Mints still within their active tracking window (worth refreshing)."""
    cutoff = int(time.time()) - max_age_hours * 3600
    conn = _get_conn()
    with _DB_LOCK:
        rows = conn.execute(
            "SELECT mint FROM token_lifecycle WHERE first_ts >= ? AND status='active' AND chain=?",
            (cutoff, chain)).fetchall()
    return [r[0] for r in rows]


def get_open_trade_mints(chain: str = "SOL") -> list:
    ...
    with _DB_LOCK:
        rows = conn.execute(
            ...
            "AND status='active' AND chain=?", (chain,)).fetchall()
    return [r[0] for r in rows]
...
def run_risk_monitor(price_fetcher, on_drawdown=None, on_stop_loss=None, on_rug=None, on_tp_hit=None,
    ...
    `chain` scopes which chain's open trades get checked — pass a
    chain-specific `price_fetcher` alongside it (e.g. the BNB price fetcher
    for chain="BNB"), since a Solana price fetcher can't price a BNB token.
    ...
    """
    mints = get_open_trade_mints(chain=chain)
    if not mints:
        return
    logger.info("Risk monitor [%s]: checking %d open trades", chain, len(mints))
    for mint in mints:
        try:
            price_data = price_fetcher(mint, bypass_cache=True)
            price = price_data.get("price") or 0
            ...
            updated = upsert_token_lifecycle(
                ...
                price_change_5m=price_data.get("price_change_5m"), chain=chain)
            ...
            # Checked last, deliberately — rug is the most severe/alarming
            # signal, and by this point it's already been confirmed across
            # RUG_CONFIRM_STRIKES consecutive passes and wasn't contradicted
            # by a TP hit in the same pass (see upsert_token_lifecycle).
...
def record_buy_sequence(mint: str, wallet: str, buy_ts: int, chain: str = "SOL") -> int:
    """
    ...
    with _DB_LOCK:
        ...
        conn.execute(
            "INSERT INTO buy_sequences (mint, wallet, buy_ts, seq_position, chain) VALUES (?,?,?,?,?)",
            (mint, wallet, buy_ts, pos, chain))
        conn.commit()
    return pos


def compute_leader_follower_edges(mint: str, chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        for i, (leader, leader_ts) in enumerate(rows):
            for follower, follower_ts in rows[i + 1:]:
                ...
                conn.execute(
                    "INSERT INTO leader_edges (leader, follower, cooccurrences, led_count, chain) "
                    "VALUES (?,?,1,1,?) "
...
def record_trade_outcome_extended(wallet: str, win: bool, roi_pct: float, chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        ...
        conn.execute(
            "INSERT INTO wallet_metrics (wallet, trades, wins, losses, total_roi, roi_samples, chain) "
            "VALUES (?,?,?,?,?,?,?) "
            ...
            (wallet, trades, wins, losses, total_roi, samples_str, chain))
        conn.commit()

    recompute_wallet_confidence(wallet, chain=chain)
...
def recompute_wallet_confidence(wallet: str, chain: str = "SOL") -> dict:
    """
    ...
    if not row or row[0] < MIN_TRADES_FOR_CONFIDENCE:
        ...
        _store_confidence(wallet, confidence, 0, 0, early_score, leader_score, 50,
                          trades_known=trades, chain=chain)
    ...
    # Rug-history penalty — a wallet that keeps buying into rugs shouldn't
    # keep full weight just because its OTHER trades look fine on paper.
    # Capped so a couple of early rugs (common while the roster is still
    # learning) don't permanently tank a wallet that's otherwise solid.
    try:
        rugs = compute_wallet_quality_metrics(wallet).get("rugs_bought", 0)
        rug_penalty = min(rugs * 4, 20)
        confidence = max(0, confidence - rug_penalty)
    except Exception as e:
        logger.debug("Rug penalty lookup failed for %s: %s", wallet, e)

    confidence = round(confidence, 1)

    _store_confidence(wallet, confidence, roi_score, hit_rate_score, early_score,
                      leader_score, consistency, trades_known=trades, chain=chain)
...
def _store_confidence(wallet, confidence, roi_score, hit_rate_score, early_score,
                      leader_score, consistency, trades_known, chain: str = "SOL"):
    ...
    with _DB_LOCK:
        ...
        conn.execute(
            ...
            "early_entry_score, leader_score, consistency_score, tier, last_updated, chain) "
            "VALUES (?,?,?,?,?,?,?,?,?,?) "
            ...
            (wallet, confidence, roi_score, hit_rate_score, early_score, leader_score,
             consistency, tier, int(time.time()), chain))
        conn.commit()
...
# Explicit, predictable weights for the alert-triggering weighted vote — more
# interpretable than a continuous confidence score: an Elite wallet always
# counts 3x a Standard wallet, Probation counts for half, Retired doesn't
# count at all, regardless of the exact underlying confidence number. This
# is intentionally a DIFFERENT mechanism from get_wallet_confidence_score
# (which still drives the continuous 0-100 score shown for display, and the
# consensus score's wallet_quality component) — this one is specifically
# for "how many votes does this wallet's buy contribute toward a signal."
TIER_WEIGHTS = {
    "ELITE":     3.0,
    "STRONG":    2.0,
    "STANDARD":  1.0,
...
def get_wallet_tier_weight(wallet: str) -> float:
    """Fixed weight for a wallet based on its current promotion/demotion tier."""
    return TIER_WEIGHTS.get(get_wallet_tier(wallet), 1.0)
...
def seed_active_roster(wallets: list, chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        for w in wallets:
            conn.execute(
                "INSERT OR IGNORE INTO active_roster (wallet, added_ts, source, status, chain) "
                "VALUES (?,?,'seed','active',?)",
                (w, now, chain))
        conn.commit()


def reconcile_seed_roster(wallets: list, chain: str = "SOL") -> int:
    """
    The other half of seed_active_roster(): retires any wallet that's
    currently active with source='seed' but is NO LONGER in `wallets` — i.e.
    it was removed from the wallet list in code. Without this, editing that
    list does nothing at runtime: seed_active_roster only ever adds, so an
    old wallet you deleted from the code stays active in the DB forever and
    keeps getting sent to the webhook.

    IMPORTANT: filtered by `chain` — without this, reconciling one chain's
    wallet list would see the OTHER chain's wallets as "not in this list"
    and incorrectly retire them.

    Only touches source='seed' rows — wallets the discovery engine added on
    its own (source='discovery') are left alone, since they're expected to
    exist outside the static list. Returns the number of wallets retired.
    """
    conn = _get_conn()
    current = set(wallets)
    with _DB_LOCK:
        rows = conn.execute(
            "SELECT wallet FROM active_roster WHERE status='active' AND source='seed' AND chain=?",
            (chain,)).fetchall()
        stale = [r[0] for r in rows if r[0] not in current]
        for w in stale:
            conn.execute(
                "UPDATE active_roster SET status='retired' WHERE wallet=? AND chain=?", (w, chain))
        if stale:
            conn.commit()
    if stale:
        logger.info("🪦 [%s] Retired %d wallet(s) removed from wallet list: %s",
                   chain, len(stale), ", ".join(w[:8]+"…" for w in stale))
    return len(stale)


def get_active_roster(chain: str = "SOL") -> list:
    conn = _get_conn()
    with _DB_LOCK:
        rows = conn.execute(
            "SELECT wallet FROM active_roster WHERE status='active' AND chain=?", (chain,)).fetchall()
    return [r[0] for r in rows]
...
def compute_wallet_quality_metrics(wallet: str, window_days: int = 30) -> dict:
    """
    The extended wallet quality score, built from wallet_trade_log:
      - win_rate_30d      : % of resolved trades in the trailing window that won
      - avg_roi_pct       : average ROI% across resolved trades in the window
      - median_hold_secs  : median (resolved_ts - alert_ts) across resolved trades
      - rugs_bought       : count of trades that resolved as outcome_type='rug'
      - avg_entry_mcap    : average market cap at the moment this wallet's buy fired
      - realized_pnl_sol  : sum(sol_spent * roi_pct/100) — a SOL-denominated PnL
                            estimate. Approximate, not exact accounting: it uses
                            the SOL committed at entry and the token's realized
                            ROI%, not an actual on-chain exit fill price.
    Trades still awaiting resolution (resolved_ts IS NULL) are excluded from
    every metric except avg_entry_mcap, which counts all trades regardless of
    whether they've resolved yet.
    """
    since_ts = int(time.time()) - window_days * 86400
    conn = _get_conn()
    with _DB_LOCK:
        resolved = conn.execute(
            "SELECT roi_pct, win, outcome_type, sol_spent, alert_ts, resolved_ts "
            "FROM wallet_trade_log "
            "WHERE wallet=? AND alert_ts>=? AND resolved_ts IS NOT NULL",
            (wallet, since_ts)).fetchall()
        all_entries = conn.execute(
            "SELECT entry_mcap FROM wallet_trade_log WHERE wallet=? AND alert_ts>=?",
            (wallet, since_ts)).fetchall()

    if not resolved:
        return {
            "win_rate_30d": None, "avg_roi_pct": None, "median_hold_secs": None,
            "rugs_bought": 0, "avg_entry_mcap": None, "realized_pnl_sol": 0.0,
            "resolved_trades": 0,
        }

    wins       = sum(1 for r in resolved if r[1])
    roi_vals   = [r[0] for r in resolved if r[0] is not None]
    rugs       = sum(1 for r in resolved if r[2] == "rug")
    hold_times = sorted((r[5] - r[4]) for r in resolved if r[4] and r[5])
    pnl_sol    = sum((r[3] or 0) * (r[0] or 0) / 100 for r in resolved)

    mcaps = [e[0] for e in all_entries if e[0]]
    median_hold = hold_times[len(hold_times)//2] if hold_times else None

    return {
        "win_rate_30d":     round(wins / len(resolved) * 100, 1),
        "avg_roi_pct":      round(sum(roi_vals) / len(roi_vals), 1) if roi_vals else None,
        "median_hold_secs": median_hold,
        "rugs_bought":      rugs,
        "avg_entry_mcap":   round(sum(mcaps) / len(mcaps), 0) if mcaps else None,
        "realized_pnl_sol": round(pnl_sol, 3),
        "resolved_trades":  len(resolved),
    }


def classify_wallet_style(wallet: str) -> str:
    """
    Classifies a wallet's trading style using data this bot actually has:
    early-entry tendency (from the existing early-entry score) and hold
    time (from wallet_trade_log). Returns one of:
      "Early Hunter"    — consistently buys very soon after launch
      "Momentum Trader"  — short median hold time, in and out fast
      "Swing Holder"     — long median hold time, rides positions out
      "Standard"         — doesn't strongly fit either pattern
      "New"              — not enough resolved trades yet to classify

    Note: a 4th category ("Distribution Expert" — wallets that scale out in
    stages rather than exiting all at once) was in the original spec but
    isn't implemented here — this bot doesn't currently track partial-sell
    amounts per wallet, only full buy/sell events, so there isn't a reliable
    signal to classify that behavior from yet.
    """
    metrics = compute_wallet_quality_metrics(wallet)
    if metrics["resolved_trades"] < 3:
        return "New"

    early_score = get_wallet_early_entry_score(wallet)
    median_hold = metrics.get("median_hold_secs")

    if early_score >= 75:
        return "Early Hunter"
    if median_hold is not None and median_hold < 900:       # < 15 min
        return "Momentum Trader"
    if median_hold is not None and median_hold > 21600:      # > 6 hours
        return "Swing Holder"
    return "Standard"
...
def _score_bot_coordination(buy_times, sol_amounts) -> float:
    """
    0-100 — how organic this convergence looks. Independent wallets making
    their own decisions rarely buy in the exact same second with near-
    identical SOL sizes; when several do, it's a signature of a single
    bundled transaction, a sniper/MEV bot cluster, or wash trading rather
    than genuine independent consensus. 100 = no red flags detected; lower
    values dampen the overall consensus score (see compute_consensus_score).
    """
    if len(buy_times) < 2:
        return 100.0
    ...
    # Same-second clustering — 2-3+ buys landing in the identical 1s bucket.
    buckets = {}
    for t in buy_times:
        buckets[int(t)] = buckets.get(int(t), 0) + 1
    max_cluster = max(buckets.values())
    if max_cluster >= 3:
        penalty += 35
    elif max_cluster >= 2:
        penalty += 15

    # Near-identical SOL sizes across wallets — low coefficient of variation.
    amounts = [a for a in (sol_amounts or []) if a and a > 0]
    if len(amounts) >= 3:
...
def compute_consensus_score(wallets, buy_times, sol_amounts, liquidity_usd) -> dict:
    ...
    bot_score    = _score_bot_coordination(buy_times, sol_amounts)
    ...
    # Bot-coordination is a dampening MULTIPLIER, not another weighted
    # average component — a convergence that looks bot-driven should pull
    # the whole score down proportionally, not just nudge one input.
    score = score * (bot_score / 100)
...
def _score_volume_acceleration(volume_5m, liquidity_usd) -> float:
    """
    0-100 proxy for 'is trading interest accelerating'. True acceleration
    would need a volume history to compare against (this bot doesn't
    currently persist historical volume snapshots per token), so this uses
    volume-relative-to-liquidity as a stand-in: a token doing heavy 5-minute
    volume relative to its own pool size is a reasonable signal of building
    momentum even without a time-series comparison. Approximate, not exact.
    """
    if not volume_5m or not liquidity_usd or liquidity_usd <= 0:
        return 50.0   # unknown — neutral
    ratio = volume_5m / liquidity_usd
    if ratio >= 2.0:   return 95.0
    if ratio >= 1.0:   return 82.0
    if ratio >= 0.5:   return 68.0
    if ratio >= 0.2:   return 52.0
    if ratio >= 0.05:  return 38.0
    return 22.0


def compute_token_quality_score(liquidity_usd, mcap, holder_concentration_pct,
                                consensus_score, token_age_secs, buy_pressure_ratio,
                                volume_5m=None) -> dict:
    """
    ...
    concentration, market cap, wallet consensus, token age, buy pressure,
    and a volume-acceleration proxy.
    """
    ...
    vol_s   = _score_volume_acceleration(volume_5m, liquidity_usd)
    ...
    score = (
        ...
        vol_s   * TOKEN_QUALITY_WEIGHTS["volume_acceleration"]
    )
    return {
        "score": round(score, 1),
        "breakdown": {
...
def start_alert_outcome_tracking(mint: str, wallets, entry_price: float, entry_mcap: float = None,
                                 sol_by_wallet: dict = None, chain: str = "SOL"):
    """
    Call once, right when a buy alert fires. `sol_by_wallet`, if given, is a
    {wallet: sol_amount} map used to seed wallet_trade_log — the per-wallet
    record that the extended wallet quality score (hold time, rugs bought,
    avg entry mcap, realized PnL) is built from.
    """
    ...
    with _DB_LOCK:
        conn.execute(
            ...
            "running_high, running_low, last_price, last_updated, status, chain) "
            "VALUES (?,?,?,?,?,?,?,?,?,'tracking',?) "
            ...
            (mint, wallets_str, now, entry_price, entry_mcap,
             entry_price, entry_price, entry_price, now, chain))

        sol_by_wallet = sol_by_wallet or {}
        for w in set(wallets):
            try:
                conn.execute(
                    "INSERT OR IGNORE INTO wallet_trade_log "
                    "(wallet, mint, entry_mcap, sol_spent, alert_ts, chain) VALUES (?,?,?,?,?,?)",
                    (w, mint, entry_mcap, sol_by_wallet.get(w), now, chain))
            except Exception as e:
                logger.debug("wallet_trade_log insert failed for %s/%s: %s", w, mint, e)
        conn.commit()
...
def _finalize_checkpoint(conn, mint, wallets_str, entry_price, high, low, current_price,
                         checkpoint_min: int, chain: str = "SOL") -> dict:
    ...
    if checkpoint_min == 60:
        ...
        record_outcome_event(mint, "checkpoint_60m", roi_pct, win, chain=chain)

    return result


def run_alert_outcome_checkpoints(chain: str = "SOL") -> list:
    """
    ...
    Filtered by `chain` — each chain's scheduler pass only finalizes its own
    alerts.
    """
    now = int(time.time())
    conn = _get_conn()
    ...
    with _DB_LOCK:
        rows = conn.execute(
            ...
            "FROM alert_outcomes WHERE status='tracking' AND chain=?", (chain,)).fetchall()
        ...
        for (mint, wallets_str, alert_ts, entry_price, high, low, last_price,
            ...
            if not c15 and age_min >= 15:
                completed.append(_finalize_checkpoint(
                    conn, mint, wallets_str, entry_price, high, low, current_price, 15, chain=chain))
            if not c30 and age_min >= 30:
                completed.append(_finalize_checkpoint(
                    conn, mint, wallets_str, entry_price, high, low, current_price, 30, chain=chain))
            if not c60 and age_min >= 60:
                completed.append(_finalize_checkpoint(
                    conn, mint, wallets_str, entry_price, high, low, current_price, 60, chain=chain))
...
def get_tracking_alert_mints(chain: str = "SOL") -> list:
    """Mints with an alert-outcome record still awaiting its 60-min checkpoint."""
    conn = _get_conn()
    with _DB_LOCK:
        rows = conn.execute(
            "SELECT mint FROM alert_outcomes WHERE status='tracking' AND chain=?", (chain,)).fetchall()
    return [r[0] for r in rows]


def refresh_alert_outcome_prices(price_fetcher, chain: str = "SOL"):
    """
    ...
    `price_fetcher(mint, bypass_cache=True) -> dict` — same shape as
    `_get_token_price`, needs at least "price". Must be the chain-appropriate
    fetcher for `chain` — a Solana price fetcher can't price a BNB token.
    """
    mints = get_tracking_alert_mints(chain=chain)
    for mint in mints:
        try:
            price_data = price_fetcher(mint, bypass_cache=True)
            price = price_data.get("price") or 0
...
def record_outcome_event(mint: str, outcome_type: str, roi_pct: float, win: bool, chain: str = "SOL"):
    """
    ...
    with _DB_LOCK:
        ...
        conn.execute(
            "INSERT INTO outcome_log (mint, ts, outcome_type, roi_pct, win, chain) VALUES (?,?,?,?,?,?)",
            (mint, resolved_ts, outcome_type, roi_pct, 1 if win else 0, chain))
        if row:
            conn.execute("UPDATE alert_outcomes SET status='complete' WHERE mint=?", (mint,))
        conn.execute(
            "UPDATE wallet_trade_log SET resolved_ts=?, outcome_type=?, roi_pct=?, win=? "
            "WHERE mint=? AND resolved_ts IS NULL",
            (resolved_ts, outcome_type, roi_pct, 1 if win else 0, mint))
        conn.commit()

    for w in wallets:
        try:
            record_trade_outcome_extended(w, win=win, roi_pct=roi_pct, chain=chain)
        except Exception as e:
...
def set_setting(key: str, value: str):
    """Generic key-value persistence — e.g. the registered Helius webhook ID."""
    conn = _get_conn()
    with _DB_LOCK:
        conn.execute(
