...
WINDOW           = 180    # seconds — buy convergence window (3 minutes)
    ...
    for transfer in sold:
        ...
        with activity_lock:
            ...
            if not already_exit_alerted:
                ...
                # the terminal signal and still respects MIN_HOLD_TIME unless
                # it's a fast coordinated dump. ─────────────────────────────
                is_exit_now = (
                    sell_count >= _get_adaptive_threshold() or
                    overlap_ratio >= 0.5 or
                    fast_dump
                )
                exit_now_ready = is_exit_now and (not within_hold_window or fast_dump)

                if exit_now_ready:
                    severity = "exit_now"
                elif overlap_ratio >= 0.34 or sell_count >= 2:
                    severity = "reduce"
                elif sell_count >= 1:
