...
- Confirmation wait: once a token qualifies, waits CONFIRMATION_WAIT_SECONDS before
  grading/sending — cancelled if a tracked wallet sells or price drops
  CANCEL_PRICE_DROP_PCT+ from first detection during the wait
- Elite wallets: 3 hand-picked wallets get an instant standalone alert on every buy
  (any token). Elite sells only alert for tokens that already got a BUY SIGNAL — an
  Elite wallet quietly exiting something that was never flagged isn't actionable.
...
# ── Buy-signal confirmation / cancellation ─────────────────────────────────
# Once a token qualifies for a full graded BUY SIGNAL, it isn't sent right
# away — it waits CONFIRMATION_WAIT_SECONDS first. If, during that wait, any
# of the wallets that triggered the signal starts selling the token, or the
# price falls CANCEL_PRICE_DROP_PCT or more from the price at first
# detection, the signal is cancelled and nothing is sent. See
# _confirm_and_send_buy() / _finalize_buy_signal().
CONFIRMATION_WAIT_SECONDS = 90    # seconds to wait before confirming + sending
CANCEL_PRICE_DROP_PCT     = 15    # cancel if price drops this % or more (15-20% range) from first detection
...
MIN_GRADE_SCORE     = 65     # suppress buy alerts below this AI grade score (0-100) — A+/A/B/C/D
                              # grade shown in alerts is derived from this same score, see _grade_signal()
...
# ── Elite wallets ────────────────────────────────────────────────────────────
# Hand-picked, fixed roster (independent of the computed ELITE/STRONG/etc tier
# system in wallet_intelligence.py — that tier is earned dynamically, this is
# a manual designation). Every buy from one of these 3 fires its own instant
# "ELITE WALLET BUY" alert (see _send_elite_buy_alert), on top of counting
# normally toward the consensus/convergence signal like any other wallet.
ELITE_WALLETS = [
...
# Full active roster = Elite + Standard. Elite wallets are NOT tracked
# separately from the consensus/webhook pipeline — they're simply the first
# 3 entries here, which also count toward convergence like everyone else.
FALLBACK_WALLETS = ELITE_WALLETS + STANDARD_WALLETS

# Fast membership checks for the per-buy "is this an Elite wallet" test.
ELITE_WALLET_SET = set(ELITE_WALLETS)
# ═══════════════════════════════════════════════════════════════════════════════
#  INTERNALS
# ═══════════════════════════════════════════════════════════════════════════════
...
early_alerted   = TTLCache(maxsize=1000, ttl=WINDOW * 2)
sell_activity   = TTLCache(maxsize=1000, ttl=SELL_WINDOW * 2)
sell_alerted    = TTLCache(maxsize=1000, ttl=ALERT_COOLDOWN)
# Every sell observed for a mint, recorded unconditionally (unlike
# sell_activity, which only accumulates once a buy alert already exists).
# This is what lets a pending BUY SIGNAL — still inside its confirmation
# wait — see a tracked wallet bail out before the signal has even fired.
# {mint: [(wallet, ts), ...]} — access under recent_sells_lock.
recent_sells      = TTLCache(maxsize=1000, ttl=WINDOW + CONFIRMATION_WAIT_SECONDS + 60)
recent_sells_lock = threading.Lock()
...
# consensus update) on something that isn't a new signal. Also reused for
# the sell side (keys prefixed "sell:") for the same reason — see the SELL
# detection block.
...
def _send_elite_buy_alert(wallet: str, mint: str, symbol: str, sol_spent: float, ts: int):
    """
    Instant standalone alert fired whenever ANY of the 3 hand-picked Elite
    wallets (ELITE_WALLETS) buys a token — independent of the normal
    consensus/convergence threshold, and independent of the confirmation
    wait period that gates the graded BUY SIGNAL. One Elite wallet buying is
    signal enough on its own to be worth a heads-up immediately.
    """
    try:
        price_data = _get_token_price(mint)
        price_str  = price_data.get("price_str", "N/A")
        mcap_str   = price_data.get("mcap_str", "N/A")
        time_str   = time.strftime("%H:%M:%S UTC", time.gmtime(ts))
        _send_message_safe(
            chat_id=CHAT_ID,
            text=(
                f"👑 <b>ELITE WALLET BUY</b>\n\n"
                f"🪙 Token   : <b>{_esc(symbol)}</b>\n"
                f"👛 Wallet  : <code>{_esc(wallet)}</code>\n"
                f"💰 SOL Spent : <b>{sol_spent:.2f}</b>\n"
                f"💵 Price   : {price_str}\n"
                f"🏷 MCap    : {mcap_str}\n"
                f"🕐 Time    : {time_str}\n"
                f"📋 CA: <code>{_esc(mint)}</code>"
        ...
        logger.debug("Elite-wallet buy alert failed for %s/%s: %s", wallet, mint, e)
    except Exception as e:
        logger.debug("Elite-wallet buy alert error for %s/%s: %s", wallet, mint, e)


def _send_elite_sell_alert(wallet: str, mint: str, symbol: str, sol_received: float, ts: int):
    """
    Mirror of _send_elite_buy_alert for the sell side. Instant standalone
    alert fired whenever ANY of the 3 hand-picked Elite wallets sells a
    token — but ONLY for tokens that already had a BUY SIGNAL sent (see the
    buy_alert_wallets check at the call site). A token that never qualified
    for a buy alert isn't one we've told you to watch, so an Elite wallet
    quietly exiting it isn't actionable. Independent of the normal exit
    severity ladder (take_profit/reduce/exit_now), which only tracks
    wallets that were part of the original triggering buy — this fires for
    ANY Elite sell on an alerted token, even if that Elite wallet wasn't
    one of the original buyers.
    """
    try:
        price_data = _get_token_price(mint)
        price_str  = price_data.get("price_str", "N/A")
        ...
        _send_message_safe(
            chat_id=CHAT_ID,
            text=(
                f"👑 <b>ELITE WALLET SELL</b>\n\n"
                f"🪙 Token   : <b>{_esc(symbol)}</b>\n"
                f"👛 Wallet  : <code>{_esc(wallet)}</code>\n"
                f"💰 SOL Received : <b>{sol_received:.2f}</b>\n"
                f"💵 Price   : {price_str}\n"
                f"🏷 MCap    : {mcap_str}\n"
                f"🕐 Time    : {time_str}\n"
                f"📋 CA: <code>{_esc(mint)}</code>"
        ...
        logger.debug("Elite-wallet sell alert failed for %s/%s: %s", wallet, mint, e)
    except Exception as e:
        logger.debug("Elite-wallet sell alert error for %s/%s: %s", wallet, mint, e)
...
def _extract_sol_received(tx: dict, wallet: str) -> float:
    """
    Mirror of _extract_sol_spent for the sell side — sum of native SOL and
    WSOL transfers RECEIVED by the wallet in this swap tx. Used for the
    Elite wallet sell alert's "SOL Received" figure. Same fee/rounding
    caveat as _extract_sol_spent: approximate, not accounting-grade.
    """
    total = 0.0
    for nt in tx.get("nativeTransfers", []) or []:
...
def _confirm_and_send_buy(mint: str, symbol: str, trigger_ts: int, unique: frozenset,
                          price_data: dict, buy_times: list, sol_amounts: list,
                          adaptive_thresh: int, entries: list, first_price):
    """
    Runs CONFIRMATION_WAIT_SECONDS after a token first qualifies for a BUY
    SIGNAL, then re-validates before grading/sending anything. Two
    cancellation conditions, either one kills the signal outright:

      1. Any wallet that was part of the triggering convergence starts
         selling this token before the wait is up (checked against
         recent_sells, which records every sell unconditionally — not just
         ones tied to an already-sent buy alert).
      2. Price has fallen CANCEL_PRICE_DROP_PCT or more from first_price
         (the price at the moment the signal first qualified).

    If neither condition trips, hands off to _finalize_buy_signal to run the
    normal rug-check/grade/quality-gate/send pipeline.
    """
    try:
        time.sleep(CONFIRMATION_WAIT_SECONDS)

        # ── Cancel condition 1: a tracked wallet defected (started selling) ──
        with recent_sells_lock:
            try:    sells = list(recent_sells[mint])
            except Exception: sells = []
        defectors = {w for (w, t) in sells if t >= trigger_ts and w in unique}
        if defectors:
            logger.info(
                "🛑 BUY SIGNAL CANCELLED — %s (%s…): %d tracked wallet(s) sold during the %ds wait period",
                symbol, mint[:8], len(defectors), CONFIRMATION_WAIT_SECONDS)
            with activity_lock:
                try:    del alerted_tokens[mint]
                except KeyError: pass
            return

        # ── Cancel condition 2: price dropped too far off first detection ──
        fresh_price_data = _get_token_price(mint, bypass_cache=True)
        fresh_price = fresh_price_data.get("price")
        if first_price and fresh_price:
            drop_pct = (first_price - fresh_price) / first_price * 100
            if drop_pct >= CANCEL_PRICE_DROP_PCT:
                logger.info(
                    "🛑 BUY SIGNAL CANCELLED — %s (%s…): price down %.1f%% from first detection during the %ds wait period",
                    symbol, mint[:8], drop_pct, CONFIRMATION_WAIT_SECONDS)
                with activity_lock:
                    try:    del alerted_tokens[mint]
                    except KeyError: pass
                return
            price_data = fresh_price_data   # use the fresher snapshot for grading

        _finalize_buy_signal(mint, symbol, trigger_ts, unique, price_data,
                             buy_times, sol_amounts, adaptive_thresh, entries)
    except Exception as e:
        logger.error("_confirm_and_send_buy error for %s: %s", mint, e, exc_info=True)
        with activity_lock:
            try:    del alerted_tokens[mint]
            except KeyError: pass


def _finalize_buy_signal(mint: str, symbol: str, ts: int, unique: frozenset, price_data: dict,
                         buy_times: list, sol_amounts: list, adaptive_thresh: int, entries: list):
    """
    The actual grading/quality-gate/send pipeline for a BUY SIGNAL that has
    survived the confirmation wait period unchanged from the pre-wait-period
    behavior, just extracted into its own function so it can be invoked
    after a delay instead of inline.
    """
...
def _process_tx(tx: dict, ts: int):
    ...
    for transfer in bought:
        ...
        # ── Elite wallet instant alert — fires on every Elite buy regardless
        # of consensus/threshold. Dispatched via the executor (not blocking)
        # since it does its own price lookup and Telegram send. ────────────
        if wallet in ELITE_WALLET_SET:
            _tx_executor.submit(_send_elite_buy_alert, wallet, mint, symbol, sol_spent, ts)
        ...
        if should_buy_alert:
            ...
            # rejected and re-triggers later as more wallets join.
            if len(unique) >= EARLY_ALERT_MIN_WALLETS and mint not in early_alerted:
            ...
            if mcap is not None and mcap > MAX_MCAP:
                logger.info(f"Rejected: Market Cap {_fmt_compact(mcap)} — {symbol} ({mint[:8]}…)")
                with activity_lock:
                    try:    del alerted_tokens[mint]
                    except KeyError: pass
            ...
            if should_buy_alert:
                # ── Confirmation wait period ────────────────────────────────
                # Don't grade/send yet. Hand off to _confirm_and_send_buy,
                # which waits CONFIRMATION_WAIT_SECONDS, re-checks that none
                # of these wallets have started selling and that price hasn't
                # dropped CANCEL_PRICE_DROP_PCT+ from right now (the first
                # detected price), and only then runs the rest of the
                # grading/quality-gate/send pipeline in _finalize_buy_signal.
                # Dispatched to the executor so this webhook-processing
                # thread isn't tied up for the whole wait.
                first_price = price_data.get("price")
                logger.info(
                    "⏳ BUY SIGNAL pending confirmation (%ds) — %s (%d wallets)",
                    CONFIRMATION_WAIT_SECONDS, symbol, len(unique))
                _tx_executor.submit(
                    _confirm_and_send_buy, mint, symbol, ts,
                    frozenset(unique), dict(price_data), list(buy_times),
                    list(sol_amounts), adaptive_thresh, list(entries), first_price)
    ...
    # Multi-hop sells can produce several separate transfer legs debiting the
    # SAME mint from the same wallet within one transaction (routing across
    # multiple pools) — the exact mirror of the "bought" consolidation above.
    # Without this, one real sell was being iterated 2-3x below, each pass
    # computing the same tx-level SOL-received total and firing an identical
    # duplicate Elite sell alert (and inflating sell_activity/holdings).
    if len(sold) > 1:
        by_mint = {}
        for t in sold:
            m = t.get("mint", "")
    ...
    for transfer in sold:
        ...
        # Short-lived (wallet, mint) dedup for sells — same rationale as the
        # buy-side recent_wallet_mint_buys: catches near-duplicate webhook
        # deliveries across different signatures that survive the above
        # same-tx consolidation.
        sell_dedup_key = f"sell:{wallet}:{mint}"
        with wallet_mint_dedup_lock:
            if sell_dedup_key in recent_wallet_mint_buys:
                logger.debug("Duplicate (wallet, mint) sell skipped: %s", sell_dedup_key)
                continue
            recent_wallet_mint_buys[sell_dedup_key] = True
        ...
        # Recorded unconditionally — this is what lets a BUY SIGNAL still
        # inside its confirmation wait period (see _confirm_and_send_buy)
        # detect a tracked wallet selling before any buy alert has been sent
        # for this mint at all.
        with recent_sells_lock:
            try:    existing_sells_all = recent_sells[mint]
            except Exception: existing_sells_all = []
            existing_sells_all.append((wallet, ts))
            recent_sells[mint] = existing_sells_all

        # Only proceed further (severity ladder + Elite sell alert) if we've
        # already sent a BUY SIGNAL for this token. A token that never
        # qualified for a buy alert isn't something we've told you to watch,
        # so an Elite wallet quietly exiting it isn't actionable — skip it.
        with buy_alert_lock:
        ...
        # ── Elite wallet instant sell alert — fires on every Elite sell of a
        # token that DID get a buy alert, regardless of whether that Elite
        # wallet itself was one of the original triggering buyers.
        # Dispatched via the executor (not blocking) since it does its own
        # price lookup and Telegram send. ──────────────────────────────────
        if wallet in ELITE_WALLET_SET:
            sol_received = _extract_sol_received(tx, wallet)
            _tx_executor.submit(_send_elite_sell_alert, wallet, mint, symbol, sol_received, ts)
        ...
        # separate alerts for the same event.
...
def list_wallets():
    ...
    return jsonify({
        "count":       len(wl),
        "elite_count": sum(1 for w in wl if w in ELITE_WALLET_SET),
        "wallets":     [{"wallet": w, "elite": w in ELITE_WALLET_SET} for w in wl],
    }), 200
