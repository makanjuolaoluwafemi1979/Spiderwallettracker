...
- Elite wallets: 3 hand-picked wallets get an instant standalone alert on every buy AND
  every sell, independent of consensus/threshold or whether a buy signal ever fired,
  on top of counting normally toward consensus
...
def _send_elite_sell_alert(wallet: str, mint: str, symbol: str, sol_received: float, ts: int):
    ...
    token — completely independent of whether a BUY SIGNAL was ever sent
    for that token, and independent of the normal exit severity ladder
    (take_profit/reduce/exit_now), which only tracks wallets that were part
    of an already-alerted buy signal. One Elite wallet selling is signal
    enough on its own to be worth a heads-up immediately.
    """
    try:
        price_data = _get_token_price(mint)
        price_str  = price_data.get("price_str", "N/A")
...
def _process_tx(tx: dict, ts: int):
    ...
    for transfer in sold:
        ...
        # ── Elite wallet instant sell alert — fires on every Elite sell,
        # completely independent of whether a buy signal was ever sent for
        # this token. Dispatched via the executor (not blocking) since it
        # does its own price lookup and Telegram send. ──────────────────
        if wallet in ELITE_WALLET_SET:
        ...
        # Only proceed further (exit-alert severity ladder) if we've already
        # sent a buy alert for this token
        with buy_alert_lock:
