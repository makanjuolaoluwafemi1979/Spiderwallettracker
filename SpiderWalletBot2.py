"""
Solana Wallet Convergence Alert Bot  — DualChainWalletBot
=======================================================
...
# Local dev / VS Code: load a .env file from the project root if present.
# On Render, env vars are injected directly and no .env file exists, so this
# is a no-op there — safe either way.
try:
...
BUY_CANCEL_DROP_PCT = 15  # cancel the graded BUY SIGNAL if price has dropped this much (%)
                          # from the first-detected price by the time grading finishes —
...
# ── Token quality filters (applied AFTER wallet convergence, BEFORE grading/alert)
# Each is env-overridable so operators can tune without redeploying.
...
# Stablecoins and other mints that are never a meaningful "buy signal" even
# though they can show up as the receive-side of a swap (e.g. SOL -> USDC,
# or a wallet parking funds in a stable before its next real buy). Filtered
# out immediately, before symbol resolution / price lookups / DB writes.
IGNORED_MINTS = {
    WSOL_MINT,
...
ELITE_WALLETS = [
    ...
    "75qsE3p5y2BFSLnfS9MUMqSAwq6MW5FhTavrUPMTFkKz",
...
FALLBACK_WALLETS = ELITE_WALLETS + [
    "2dEpwaujMteZsFYCX3hwaavYKkU7kRwAwJe9XGuWNRGA",
    "9uzxzbmrWwdxEPjY4pgPvPNjTvqQZriEb8pnXw7o4wXT",
    "DLNSLkr6bgt3umf8Ve8iJvVDGBejYi4kmcuK4yrXwKGY",
    "DaUiCLi8uFMhYmSEGf19hbfprkCg5hG3YPvVW8NPiNpS",
    "BsvjGudtAhqR5YyiZ129KwQK2tugm6u78XvgULAJpTic",
    "C937MwaPmo2d6zxysoGeEERZZit7EC7H1R3Mem3BfUKZ",
    "P6NKrKd5ZydutWqAywMYEbUV5RxiUiLevV3Vhp43ZxC",
    "DrzCkowpvjq6kjAi98EKvxw4Jt3jspc9xuVpwmhtosDh",
    "Dz7ve2Tdzt58SCctoCkNKe4MdT1VZ5vMH3iAhynFxjec",
    "6oN5Y247ZXLSwXvdQKmTQzx8V5qXQJE849PKWe8ZsyQA",
    "49zWvhEoFrCSbon9LWkqw4ipSkDbrJDKhqaNEqjTQA4v",
    "EciGzv86MdB6zLHkoLPLiAyU37BQmpJyY7yQb7SPG9zS",
    "2kW5wLFQw1wLuepZTJux1DZsTJSbNi2PGQ1E6hC7xeY8",
    "ARARAw1AL4kKnuwFyU534yoGv5ytexxeVpbcuEvpqi89",
    ...
    "GLjnQ8VT9yFHs78N5CZWGGn22a3pm4XHih5WxUpozYnq",
    "2psU7nzezVZuRjJKo7fAac14jL27MraN5V2Mkhzjw63m",
    "GWv65JohnkJgHVbqdjn4Z2BhFmBuwCTyTuUwWsspHBZ4",
    "CkakfbMYpsXd7FRNtz9mZqik1dWjk9EmmxuVhUTQcmq8",
    "6KqVAU5CW47JSKtH24vnGo5an3SAGJLZY2VQuhaD2igu",
...
# Tracks whether the instant "EARLY BUY DETECTED" heads-up has already been
# sent for a mint — separate from alerted_tokens (which gates the full,
# graded BUY SIGNAL and gets deleted/retried if a quality gate rejects the
# signal). Without this separate cache, a rejected-then-re-triggered signal
# would send a second Early Buy alert as more wallets pile on.
early_alerted   = TTLCache(maxsize=5000, ttl=86400)
# Tracks whether the Elite Exit Alert has already fired for a mint this
# trade cycle — same 24h lifetime rationale as early_alerted, so a token
# that stays "hot" for hours doesn't get repeat Elite Exit alerts either.
elite_exit_alerted = TTLCache(maxsize=5000, ttl=86400)
# Tracks whether a rejection alert has already been sent for a mint — a
# marginal signal (e.g. graded C+, just below the B- floor) can get
# re-graded and re-rejected every time a new wallet joins, since rejection
# deliberately clears the cooldown so an IMPROVING signal can still pass
# later. Without this, every one of those retries that still fails would
# send another Telegram message for the same token.
rejection_alerted = TTLCache(maxsize=5000, ttl=1800)
# ^ was TTLCache(maxsize=1000, ttl=WINDOW*2) = only 10 minutes / 1000 entries —
# nowhere near enough for a token an elite wallet keeps buying into over
# several hours. 24h + a much higher cap means "one Early Alert per token"
# actually holds for the token's whole active lifetime, not just its first
# 10 minutes.
sell_activity   = TTLCache(maxsize=1000, ttl=SELL_WINDOW * 2)
sell_alerted    = TTLCache(maxsize=1000, ttl=ALERT_COOLDOWN)
# Highest exit-severity level (1=take_profit, 2=reduce, 3=exit_now) already
# alerted per mint — lets the alert escalate as more wallets sell without
# re-sending the same tier twice. sell_alerted (above) still separately
# gates the terminal exit_now state to fully stop processing once reached.
sell_tier_alerted = TTLCache(maxsize=1000, ttl=SELL_WINDOW)
activity_lock   = threading.Lock()

# Short-lived (wallet, mint) dedup — catches multi-hop transfer legs that
# survive the same-tx mint consolidation above, near-simultaneous duplicate
# webhook deliveries across different signatures, and rapid re-buy noise,
# without burning a full processing cycle (symbol resolution, DB writes,
# consensus update) on something that isn't a new signal.
recent_wallet_mint_buys = TTLCache(maxsize=4000, ttl=45)
wallet_mint_dedup_lock  = threading.Lock()

# ── Diagnostic counters — cheap running totals so you can verify data is
# actually flowing in (webhook -> tx parsed -> buy observed) independent of
# whether a full multi-wallet convergence alert has fired yet. Surfaced on
# /status. Not persisted — resets on restart, which is fine, this is a
# "is anything happening right now" signal, not historical data.
_diag_lock = threading.Lock()
_diag = {
    "webhook_calls":   0,   # every POST to /helius-webhook, regardless of content
    "txs_received":    0,   # individual transactions across all webhook calls
    "buys_observed":   0,   # buy transfers from watched wallets, matched or not
    "last_webhook_ts": None,
    "last_buy_ts":     None,
}


def _diag_bump(key: str, ts: int = None):
    with _diag_lock:
        _diag[key] = _diag.get(key, 0) + 1
        if ts is not None:
            _diag["last_webhook_ts" if key == "webhook_calls" else "last_buy_ts"] = ts
...
def _get_weighted_vote(wallets: set) -> float:
    """
    Sum of tier weights (ELITE=3.0, STRONG=2.0, STANDARD=1.0, PROBATION=0.5,
    RETIRED=0.0) — used instead of raw wallet count for alert triggering.
    Explicit and predictable rather than a continuous confidence score: an
    Elite wallet always counts 3x a Standard one, and a Retired wallet
    contributes nothing regardless of its underlying confidence number.
    """
    try:
        return sum(wi.get_wallet_tier_weight(w) for w in wallets)
    except Exception as e:
        logger.debug("Tier-weight lookup failed, falling back to legacy score: %s", e)
        return sum(
...
def _get_adaptive_threshold() -> int:
    ...
    if count < 6:   base = THRESHOLD - 1   # quiet market — lower bar
    elif count > 7: base = THRESHOLD + 1   # active market — raise bar
    else:           base = THRESHOLD       # normal
    ...
    return max(3, min(base + adjustment, 10))
...
def _send_rejection_alert(mint: str, symbol: str, reason: str):
    """
    Sends a short Telegram notification whenever a signal clears wallet
    convergence but gets rejected by a quality gate (grade, wallet quality,
    liquidity, market cap, elite-wallet requirement, or weighted score) —
    so this is visible in chat, not just in the Render logs.

    Deduped per mint via rejection_alerted — a marginal signal can get
    re-graded and re-rejected every time a new wallet joins (rejection
    clears the cooldown so an improving signal can still pass later), and
    without this, every failed retry would send another message.
    """
    with activity_lock:
        already_sent = mint in rejection_alerted
        if not already_sent:
            rejection_alerted[mint] = True
    if already_sent:
        return
    try:
        _send_message_safe(
            ...
            text=(
                ...
                f"🪙 Token  : <b>{_esc(symbol)}</b>\n"
                f"❌ Reason : <b>{_esc(reason)}</b>\n"
                f"📋 CA: <code>{_esc(mint)}</code>"
            ),
            parse_mode="HTML",
        )
    except TelegramError as e:
        logger.debug("Rejection alert failed for %s: %s", mint, e)


def _check_buy_cancellation(mint: str, symbol: str, wallets: set, first_price):
    """
    Called right before a graded BUY SIGNAL is actually sent — grading takes
    several seconds (DexScreener, Jupiter fallback, RugCheck summary,
    RugCheck full report for holder concentration all add up), which is
    enough time for a signal to go stale before it's even alerted. Cancels
    the send if either:
      - price has already dropped BUY_CANCEL_DROP_PCT% from the
        first-detected price, or
      - any of the converging wallets has already started selling this same
        token (checked against sell_activity, independent of whether that
        sell cleared the exit-severity threshold for its own alert)
    Returns a cancellation reason string, or None if the signal is still good.
    """
    if wallets:
        with activity_lock:
            try:    recent_sells = sell_activity[mint]
            except Exception: recent_sells = []
        selling_wallets = {w for (w, _t) in recent_sells} & set(wallets)
        if selling_wallets:
            who = next(iter(selling_wallets))
            return f"wallet {who[:8]}… already selling"

    if first_price and first_price > 0:
        try:
            fresh = _get_token_price(mint, bypass_cache=True)
            current = fresh.get("price")
        except Exception as e:
            logger.debug("Cancellation price re-check failed for %s: %s", mint, e)
            current = None
        if current and current > 0:
            drop_pct = (first_price - current) / first_price * 100
            if drop_pct >= BUY_CANCEL_DROP_PCT:
                return f"price dropped {drop_pct:.1f}% since detection"
...
# Holder count cache — shares the same RugCheck full-report endpoint as
# _get_holder_concentration, but caches the *total holder count* (a single
# int) rather than the top-N concentration %. Same TTL rationale — total
# holder count only meaningfully moves over minutes, not seconds.
_total_holder_cache      = TTLCache(maxsize=2000, ttl=600)
_total_holder_cache_lock = threading.Lock()

# Authority cache — mint/freeze status from the same RugCheck payload. Kept
# separate so a hit here doesn't need to re-anchor on the whole report.
_authority_cache      = TTLCache(maxsize=2000, ttl=600)
_authority_cache_lock = threading.Lock()

# Token-age cache — pair creation timestamp from DexScreener (ms epoch).
# 10-min TTL: age only moves forward, but DexScreener occasionally backfills
# newly-indexed pairs within the first few minutes, so a short revoke window
# matters more than freshness.
_token_age_cache      = TTLCache(maxsize=2000, ttl=600)
_token_age_cache_lock = threading.Lock()

# Total pool volume cache  — DexScreener `volume.h24` (USD), used by the
# MIN_VOLUME gate. Cached so the volume check doesn't re-hit the network
# on every regrade attempt.
_pool_volume_cache      = TTLCache(maxsize=2000, ttl=600)
_pool_volume_cache_lock = threading.Lock()


def _get_total_holders(mint: str) -> int:
    """
    Total unique token-holders from the RugCheck full report endpoint.
    Returns None on any failure — callers treat None as "unknown", NOT 0,
    mirroring how missing market-cap data is handled (an unknown holder count
    shouldn't be coerced into a failing MIN_HOLDERS check).
    """
    with _total_holder_cache_lock:
        if mint in _total_holder_cache:
            return _total_holder_cache[mint]
    try:
        r = requests.get(
            f"https://api.rugcheck.xyz/v1/tokens/{mint}/report", timeout=8)
        r.raise_for_status()
        data = r.json()
        total = data.get("totalHolders")
        if total is None:
            # Some RugCheck reports only expose topHolders count — fall back
            # to that list length so we never silently disable the gate.
            top = data.get("topHolders") or []
            total = len(top) if top else 0
        else:
            total = int(total)
    except Exception as e:
        logger.debug("Holder count lookup failed for %s: %s", mint, e)
        total = None
    with _total_holder_cache_lock:
        _total_holder_cache[mint] = total
    return total


def _get_token_authorities(mint: str) -> dict:
    """
    Mint + freeze authority status (both lookups go through the same RugCheck
    full-report endpoint already used for holder concentration, but we cache
    just the two booleans so independent MIN_HOLD_TIME/MIN_AGE_SECONDS rechecks
    don't replay the whole report-parse).
    Returns dict(mint_renounced=bool, freeze_renounced=bool) — both None on
    fetch failure (treated as "unknown", never as "trust this contract").
    """
    with _authority_cache_lock:
        if mint in _authority_cache:
            return _authority_cache[mint]
    out = {"mint_renounced": None, "freeze_renounced": None}
    try:
        r = requests.get(
            f"https://api.rugcheck.xyz/v1/tokens/{mint}/report", timeout=8)
        r.raise_for_status()
        data = r.json()
        # RugCheck exposes this as `mintAuthority` / `freezeAuthority` —
        # null/missing means renounced, present means still active.
        out["mint_renounced"]   = not data.get("mintAuthority")
        out["freeze_renounced"] = not data.get("freezeAuthority")
    except Exception as e:
        logger.debug("Authority lookup failed for %s: %s", mint, e)
    with _authority_cache_lock:
        _authority_cache[mint] = out
    return out


def _get_token_age_info(mint: str) -> dict:
    """
    Pair-creation epoch (seconds) + total pool 24h volume in USD, both pulled
    from DexScreener's `latest/dex/tokens/{mint}` endpoint (we already call
    this in _fetch_dexscreener_price, so this reuses the same response shape
    rather than inventing a new API).

    Returns dict(launch_ts: int|None, pool_volume_24h_usd: float|None).
    Both fields may be None on failure — gate logic treats that as "unknown"
    and skips the corresponding check rather than coercing to 0/failing.
    """
    with _token_age_cache_lock:
        cached_age = _token_age_cache.get(mint)
    with _pool_volume_cache_lock:
        cached_vol = _pool_volume_cache.get(mint)
    if cached_age is not None and cached_vol is not None:
        return cached_age, cached_vol

    out_age = cached_age if cached_age is not None else None
    out_vol = cached_vol if cached_vol is not None else None
    try:
        r = requests.get(
        ...
        if pairs:
            best = max(pairs, key=lambda x: float(x.get("liquidity", {}).get("usd") or 0))
            out_age = out_age if out_age is not None else _parse_pair_age(best.get("pairCreatedAt"))
            vol = best.get("volume") or {}
            if isinstance(vol, dict):
                try:    out_vol = float(vol.get("h24") or 0) or None
                except Exception: pass
    except Exception as e:
        logger.debug("Token-age/volume lookup failed for %s: %s", mint, e)

    with _token_age_cache_lock:
        if out_age is not None:
            _token_age_cache[mint] = out_age
    with _pool_volume_cache_lock:
        if out_vol is not None:
            _pool_volume_cache[mint] = out_vol
    return out_age, out_vol


def _parse_pair_age(pair_created_at) -> int:
    """
    DexScreener returns pairCreatedAt as a Unix timestamp in MILLISECONDS.
    Returns seconds. Tolerates missing / malformed input cleanly.
    """
    if not pair_created_at:
        return None
    try:
        ms = int(pair_created_at)
        # If someone returns seconds instead (the rare case), it'd already
        # be a 10-digit number — MSD heuristic rather than a guess.
        return ms // 1000 if ms > 10_000_000_000 else ms
    except (TypeError, ValueError):
        return None


def _check_token_quality_filter(mint: str, symbol: str, price_data: dict,
                                rug: dict, first_detected_ts: int) -> str:
    """
    Single gate that enforces all six token-quality filters at once. Returns
    None if the token passes everything, or a short reason string otherwise
    (matching the gate-reason format used by the market-cap gate so callers
    can log + send rejection alerts with the same code path).

    Defensive by design:
      - missing data is treated as "skip that check" rather than "fail
        automatically" — same convention as the market-cap gate above.
      - REQUIRE_RENOUNCED=False (the default) completely bypasses the
        authority check, matching the requested config.
      - डेटा "unknown" never coerces to 0: an unknown holder count, unknown
        age, or unknown volume does NOT count as "below threshold", it
        counts as "we couldn't verify it, let the rest of the gates decide".

    numeric fields in price_data:
      market_cap (=None means unknown),
      liquidity_usd (=0 if DexScreener omitted it),
      volume_5m (5-minute pool volume; the requested MIN_VOLUME says
                 "tracking window" — DexScreener's main per-pair number is
                 the 5m bucket we already cache as volume_5m, so that maps
                 directly to "the last tracking window"). If 5m is missing,
                 fall back to 24h volume which is wider but still the
                 smallest-grain volume DexScreener guarantees.
    """
    mcap      = price_data.get("market_cap")
    liq_usd   = price_data.get("liquidity_usd") or 0
    vol_5m    = price_data.get("volume_5m")
    token_age_secs = None

    # ── Token age gate — MIN_AGE_SECONDS / MAX_AGE_SECONDS ────────────────
    launch_ts = wi.get_token_launch_ts(mint)
    if launch_ts:
        token_age_secs = first_detected_ts - launch_ts
    else:
        # Fall back to DexScreener pair creation timestamp if the intelligence
        # DB doesn't have a launch yet (fresh tokens may not have been
        # upserted before the first convergence event).
        ds_launch, _ = _get_token_age_info(mint)
        if ds_launch:
            token_age_secs = first_detected_ts - ds_launch

    if token_age_secs is not None:
        if token_age_secs < MIN_AGE_SECONDS:
            mins = token_age_secs // 60
            return f"Token Age {mins}m < {MIN_AGE_SECONDS // 60}m"
        if token_age_secs > MAX_AGE_SECONDS:
            mins = token_age_secs // 60
            return f"Token Age {mins}m > {MAX_AGE_SECONDS // 60}m"

    # ── Liquidity ratio gate — MIN_LIQUIDITY_RATIO ────────────────────────
    if mcap and mcap > 0 and liq_usd > 0:
        ratio = liq_usd / mcap
        if ratio < MIN_LIQUIDITY_RATIO:
            return f"Liq/Mcap {_fmt_pct(ratio)} < {_fmt_pct(MIN_LIQUIDITY_RATIO)}"

    # ── Volume gate — MIN_VOLUME (USD in the last tracking window) ────────
    if vol_5m is None:
        # rechek from DexScreener if the price fetch path didn't pick it up
        _, ds_vol = _get_token_age_info(mint)
        vol_5m = ds_vol
    if vol_5m is not None and vol_5m < MIN_VOLUME:
        return f"Volume ${_fmt_compact(vol_5m)} < ${_fmt_compact(MIN_VOLUME)}"

    # ── Holder count gate — MIN_HOLDERS ───────────────────────────────────
    holders = _get_total_holders(mint)
    if holders is not None and holders < MIN_HOLDERS:
        return f"Holders {holders} < {MIN_HOLDERS}"

    # ── Authority renounce gate — REQUIRE_RENOUNCED ────────────────────────
    # The requested default is False — i.e. authorities are NOT required to
    # be renounced. We honor that by short-circuiting when the flag is off,
    # and only check when the operator has explicitly opted in.
    if REQUIRE_RENOUNCED:
        # Reuse the rug dict if it already has the booleans (it normally
        # does, see _check_rug_risk's call to the same endpoint). Only do
        # the extra hit if missing.
        mint_off   = rug.get("mint_disabled")
        freeze_off = rug.get("freeze_disabled")
        if mint_off is None or freeze_off is None:
            auths = _get_token_authorities(mint)
            mint_off   = auths["mint_renounced"]   if mint_off   is None else mint_off
            freeze_off = auths["freeze_renounced"] if freeze_off is None else freeze_off
        # "renounced" == authority disabled
...
def _ts_footer() -> str:
    """
    Compact UTC timestamp appended to risk-monitor-driven alerts (drawdown,
    stop-loss, rug, TP hit, milestone). These all come from the same
    periodic background pass and can occasionally arrive at Telegram in a
    different order than the underlying price events actually happened in —
    this timestamp is the ground truth for reconstructing real order,
    independent of delivery sequence.
    """
    return time.strftime("%H:%M:%S UTC", time.gmtime())


def _fmt_compact(value: float) -> str:
    """Compact number formatting for log lines, e.g. 32000 -> '32k' (no $ sign)."""
    value = value or 0
...
def _fmt_pct(value: float, signed: bool = False) -> str:
    """
    Module-level percent formatter used by the token-quality gate. The original
    `_fmt_pct` is a closure inside `get_alert_outcomes_summary`, so callers
    outside that method (the gate path, log lines) need a stand-alone version.
    `signed=True` prefixes '+' on positive values (default off so it matches
    the gate's neutral phrasing like 'Liq/Mcap 12% < 20%').
    """
    if value is None:
        return "N/A"
    try:
...
def _cleanup_orphan_webhooks(keep_id: str):
    """
    Deletes any OTHER Helius webhooks pointed at our own webhookURL.
    Before webhook-ID persistence was added, every restart created a brand
    new webhook via POST rather than reusing the old one — over many
    restarts that leaves several stale webhooks all independently firing
    for the same wallets, which both wastes credits and can look exactly
    like duplicate transaction deliveries. Safe to run on every startup:
    a no-op once there's nothing left to clean up.
    """
    try:
        ...
        logger.debug("Orphan webhook check failed: %s", e)
        return

    our_url = f"{APP_URL.rstrip('/')}/helius-webhook"
    orphans = [h for h in hooks
               if h.get("webhookURL") == our_url and h.get("webhookID") != keep_id]
    for h in orphans:
        wid = h.get("webhookID")
        try:
            requests.delete(
                f"{HELIUS_BASE}/webhooks/{wid}?api-key={HELIUS_API_KEY}", timeout=10)
            logger.info("🧹 Deleted orphan webhook: %s", wid)
        except Exception as e:
            logger.warning("Failed to delete orphan webhook %s: %s", wid, e)
...
def refresh_wallets():
    ...
    try:
        ...
        wi.reconcile_seed_roster(FALLBACK_WALLETS)
    ...
    # Reuse the webhook from a previous run if we don't already have one in
    # memory (i.e. this is a fresh process start, not a periodic refresh) —
    # _upsert_webhook PUTs/updates an existing ID instead of POSTing a new
    # one, which is what actually avoids the credit cost (and orphan risk)
    # of creating a fresh webhook on every restart.
    with wallet_lock:
        if not current_webhook_id:
            persisted_id = ""
            try:
                persisted_id = wi.get_setting("helius_webhook_id", "") or ""
            except Exception as e:
                logger.debug("Could not load persisted webhook ID: %s", e)
            if persisted_id:
                current_webhook_id = persisted_id
                logger.info("♻️ Reusing persisted webhook ID: %s", persisted_id)
    ...
    if new_id:
        try:
            wi.set_setting("helius_webhook_id", new_id)
        except Exception as e:
            logger.debug("Could not persist webhook ID: %s", e)
        _cleanup_orphan_webhooks(keep_id=new_id)
    ...
    panel_lines = [
        ...
        f"DB Persisted     : {'Yes' if getattr(wi, 'DB_EXISTED_AT_BOOT', True) else 'No — FRESH DB'}",
        ...
        f"Min Market Cap   : {_fmt_mcap(MIN_MCAP)}",
    ...
    try:
        _send_message_safe(
            chat_id=CHAT_ID,
            text=(
                f"⛓️ <b>DualChainWalletBot Enterprise {BOT_VERSION}</b>\n\n"
...
def _grade_signal(wallets: set, price_data: dict, buy_times: list,
    ...
    # Bonus for exceeding threshold — deliberately floored at 1.0 so this can
    # only ever help, never hurt. For the normal consensus flow, len(wallets)
    # >= adaptive_thresh is already guaranteed before this runs, so this was
    # always a pure bonus there. But an Elite-triggered or weighted-vote-
    # triggered signal can legitimately have FEWER wallets than
    # adaptive_thresh (that's the whole point of those bypasses) — without
    # the floor, this ratio dropped below 1 and crushed an otherwise-solid
    # single Elite wallet's quality score by 60-75%, which was enough on its
    # own to push a good signal into a D grade.
    excess_ratio   = max(len(wallets) / max(adaptive_thresh, 1), 1.0)
...
def _build_caption(mint: str, symbol: str, wallets: set, price_data: dict,
                   ...
                   holdings_pct: float = None, severity: str = "exit_now") -> str:
    ...
    def _wallet_line(w: str) -> str:
        tag = ""
        try:
            style = wi.classify_wallet_style(w)
            if style not in ("New", "Standard"):
                tag = f", {style}"
        except Exception:
            pass
        return f"  <code>{w[:4]}…{w[-4:]}</code> (score: {_get_wallet_score(w):.2f}{tag})"

    preview = "\n".join(_wallet_line(w) for w in list(wallets)[:5])
    if len(wallets) > 5:
    ...
    if not is_sell:
        ...
        if trade and trade.get("available"):
            ...
            trade_block = (
                ...
                f"  • Risk only 0.5-1% of portfolio on this trade\n"
                f"  • Cap total allocation to this token at 5% of portfolio\n"
                f"  • Take partial profit around 2x to recover initial capital\n"
                f"  • Let the remainder run with a trailing exit\n"
                f"  • Exit quickly if smart money starts distributing\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
            )
    ...
    # ── Sell / Exit alert — severity reflects how much of the original
    # consensus has actually exited, not just "someone sold" ────────────────
    else:
        ...
        severity_meta = {
            "take_profit": ("🟡", "TAKE PROFIT — Leaders Scaling Out",
                            "A leading wallet is trimming — consider taking partial profits. "
                            "Not a full-exit signal yet."),
            "reduce":      ("🟠", "REDUCE POSITION — Smart Money Distributing",
                            "A meaningful share of the original buyers are exiting — "
                            "consider reducing your position."),
            "exit_now":    ("🔴", "EXIT ALERT — Same Wallets Selling",
                            "Consensus wallets have exited — exit remaining position."),
        }
        emoji, header, action = severity_meta.get(severity, severity_meta["exit_now"])
...
def _send_alert(mint: str, symbol: str, wallets: set, price_data: dict,
                ...
                holdings_pct: float = None, severity: str = "exit_now"):
    ...
    caption_full  = _build_caption(mint, symbol, wallets, price_data, is_sell,
                                    ...
                                    holdings_pct=holdings_pct, severity=severity)
...
def _process_tx(tx: dict, ts: int):
    ...
    if sig:
        with sig_lock:
            is_dup = sig in processed_signatures
            if not is_dup:
                processed_signatures[sig] = True

        if is_dup:
            logger.debug("Duplicate signature skipped: %s", sig[:12] if sig else "unknown")
            return
    ...
    bought = [t for t in transfers
              ...
              and t.get("mint", "") not in IGNORED_MINTS
              and t.get("mint", "") not in sent_non_wsol_mints]

    # Multi-hop swaps can produce several separate transfer legs crediting
    # the SAME destination mint to the same wallet within one transaction
    # (routing across multiple pools). Without this, one real buy could log
    # as 2-3 separate "buy observed" events and inflate the consensus count
    # for a single wallet. Consolidate by mint, summing amounts.
    if len(bought) > 1:
        by_mint = {}
        for t in bought:
            m = t.get("mint", "")
            if m in by_mint:
                try:
                    by_mint[m]["tokenAmount"] = (
                        float(by_mint[m].get("tokenAmount") or 0) + float(t.get("tokenAmount") or 0))
                except (TypeError, ValueError):
                    pass
            else:
                by_mint[m] = dict(t)
        bought = list(by_mint.values())
    ...
    for transfer in bought:
        ...
        # ── Skip tiny buys before doing any real work ──────────────────────
        sol_spent = _extract_sol_spent(tx, wallet)
        if sol_spent < MIN_BUY_SOL:
            logger.debug("Buy below MIN_BUY_SOL skipped: %s spent %.3f SOL on %s…",
                        wallet[:8]+"…", sol_spent, mint[:8])
            continue

        # ── Short-lived (wallet, mint) dedup — only marks buys that are
        # actually about to be processed, so a filtered-out tiny buy can't
        # accidentally block a genuine larger buy of the same token seconds
        # later. ─────────────────────────────────────────────────────────
        dedup_key = f"{wallet}:{mint}"
        with wallet_mint_dedup_lock:
            if dedup_key in recent_wallet_mint_buys:
                logger.debug("Duplicate (wallet, mint) buy skipped: %s", dedup_key)
                continue
            recent_wallet_mint_buys[dedup_key] = True

        symbol = _resolve_symbol(mint, transfer.get("tokenSymbol"))
        logger.info("💰 Buy observed — %s bought %s (%s…)", wallet[:8]+"…", symbol, mint[:8])
        _diag_bump("buys_observed", ts)
        ...
        with activity_lock:
            ...
            # A buy from one of the manually-curated ELITE_WALLETS is trusted
            # enough on its own to trigger a signal, bypassing the normal
            # wallet-count/weighted-score consensus requirement entirely —
            # while still counting toward consensus normally alongside every
            # other wallet on the list (they're in FALLBACK_WALLETS too).
            is_elite_trigger = wallet in ELITE_WALLETS
            ...
            should_buy_alert = (
                enough_wallets or enough_weight or is_elite_trigger
            ) and not already_alerted

            window_s  = int(max(buy_times) - min(buy_times)) if len(buy_times) > 1 else 0
            total_sol = sum(sol_amounts)
            # Reaching the wallet threshold isn't a guaranteed send — it still
            # has to clear the grade/wallet-quality/liquidity gates below, so
            # this only says "grading" here. The real "SENT" confirmation logs
            # right where _send_alert actually fires, further down.
            progress_tag = "\n🎯 Threshold reached — grading…" if should_buy_alert else ""
            logger.info(
                "🧩 Consensus: %s\nWallets: %d/%d\nTotal SOL: %.2f\nWindow: %ds%s",
                symbol, len(unique), adaptive_thresh, total_sol, window_s, progress_tag)

            # Only worth a distinct rejection line once it's genuinely close
            # (one wallet short) — logging this on every early buy below
            # threshold would just be noise, since that's the normal state
            # for most of a token's convergence window.
            if not should_buy_alert and len(unique) >= adaptive_thresh - 1:
                logger.info("Rejected: Weighted Score %.1f — %s (%s…)",
                           weighted_score, symbol, mint[:8])

            if should_buy_alert:
                alerted_tokens[mint] = ts

        if should_buy_alert:
            # ── Instant heads-up — sent BEFORE any of the price/rug/grading
            # network calls below, which can easily take several seconds
            # combined (DexScreener, Jupiter fallback, RugCheck summary,
            # RugCheck full report for holder concentration). Without this,
            # the full analysis message could land after the buying wallets
            # have already started selling, making it look like the bot
            # "detected the buy late" when really it just NOTIFIED late.
            # The full 🚨 BUY SIGNAL card still follows as a second message
            # once grading finishes — this one only has what's already in
            # memory, no extra I/O.
            #
            # Gated separately from the graded signal: EARLY_ALERT_MIN_WALLETS
            # is a fixed floor (not tied to the adaptive threshold, which can
            # dip to 3 in a quiet market), and early_alerted ensures this
            # fires at most ONCE per token even if the graded signal gets
            # rejected and re-triggers later as more wallets join. An elite
            # wallet's buy bypasses the floor entirely, same as the graded
            # signal above.
            if len(unique) >= EARLY_ALERT_MIN_WALLETS or is_elite_trigger:
                with activity_lock:
                    already_early_alerted = mint in early_alerted
                    if not already_early_alerted:
                        early_alerted[mint] = ts
                if not already_early_alerted:
                    elite_tag = "\n⭐ <b>Elite wallet buy</b>" if is_elite_trigger else ""
                    try:
                        time_str = time.strftime("%H:%M:%S UTC", time.gmtime(ts))
                        _send_message_safe(
                            chat_id=CHAT_ID,
                            text=(
                                f"🚨 <b>EARLY BUY DETECTED</b>{elite_tag}\n\n"
                                f"🪙 Token   : <b>{_esc(symbol)}</b>\n"
                                f"👛 Wallets : <b>{len(unique)}</b> buying\n"
                                f"💰 Total SOL : <b>{total_sol:.2f}</b>\n"
                                f"🕐 Time    : {time_str}\n"
                                f"📋 CA: <code>{_esc(mint)}</code>\n\n"
                                f"⏳ <i>Full analysis incoming…</i>"
                            ),
                            parse_mode="HTML",
                        )
                    except TelegramError as e:
                        logger.debug("Early-buy heads-up failed for %s: %s", mint, e)

            price_data = _get_token_price(mint)
            first_detected_price = price_data.get("price")

            # Market cap gate — use flag not continue so other tokens in same tx still process.
            # IMPORTANT: mcap can be None (Jupiter's fallback price API doesn't return
            # market cap at all, and DexScreener can occasionally omit it too) — that's
            # "unknown", not "zero". Coercing None to 0 here would make every
            # Jupiter-fallback-priced token fail the MIN_MCAP floor automatically,
            # rejecting good signals just because we don't have the number, not
            # because the token is actually worthless. Only enforce the gate when
            # we actually have a real value to check.
            mcap = price_data.get("market_cap")
            if mcap is not None and mcap > MAX_MCAP:
                mcap_reason = f"Market Cap {_fmt_compact(mcap)} > {_fmt_compact(MAX_MCAP)}"
                logger.info(f"Rejected: {mcap_reason} — {symbol} ({mint[:8]}…)")
                _send_rejection_alert(mint, symbol, mcap_reason)
                with activity_lock:
                    try:    del alerted_tokens[mint]
                    except KeyError: pass
                should_buy_alert = False
            elif mcap is not None and mcap < MIN_MCAP:
                mcap_reason = f"Market Cap {_fmt_compact(mcap)} < {_fmt_compact(MIN_MCAP)}"
                logger.info(f"Rejected: {mcap_reason} — {symbol} ({mint[:8]}…)")
                _send_rejection_alert(mint, symbol, mcap_reason)
                with activity_lock:
                    try:    del alerted_tokens[mint]
                ...
                logger.debug("Market cap unknown for %s (%s…) — skipping mcap gate, grading anyway",
                            symbol, mint[:8])

            # ── Token-quality filters (volume / age / holders / authority /
            # liquidity-ratio) — applied AFTER the market-cap gate but BEFORE
            # the rug check + grading, since those are heavier I/O. Cheap to
            # check first because the constants are all integers/booleans and
            # most of the lookups are shared with downstream code (the rug /
            # holder-concentration helpers below hit the same RugCheck
            # endpoint, and the gate reuses `_get_holder_concentration`'s
            # 600-second TTL caches wherever possible).
            if should_buy_alert:
                try:
                    tq_reason = _check_token_quality_filter(
                        mint, symbol, price_data, {}, ts)
                except Exception as e:
                    logger.debug("Token-quality gate failed for %s: %s", mint, e)
                    tq_reason = None
                if tq_reason:
                    logger.info("Rejected: %s — %s (%s…)", tq_reason, symbol, mint[:8])
                    _send_rejection_alert(mint, symbol, tq_reason)
                    with activity_lock:
                        try:    del alerted_tokens[mint]
                        except KeyError: pass
                    should_buy_alert = False
            ...
            if should_buy_alert:
                ...
                try:
                    ...
                    token_quality = wi.compute_token_quality_score(
                        ...
                        volume_5m=price_data.get("volume_5m"))
                ...
                if grade_data["final_score"] < MIN_GRADE_SCORE:
                    gate_reason = f"Rejected: AI Grade {grade_data['grade']}"
                elif wallet_quality < MIN_WALLET_QUALITY:
                    gate_reason = f"Rejected: Wallet Quality {wallet_quality:.0f}"
                elif liq_usd < MIN_LIQUIDITY:
                    gate_reason = f"Rejected: Liquidity {_fmt_compact(liq_usd)}"
                elif MIN_ELITE_WALLETS > 0:
                    try:
                        tiers = wi.get_wallets_by_tier(list(unique))
                        elite_n = len(tiers.get("ELITE", []))
                        if elite_n < MIN_ELITE_WALLETS:
                            gate_reason = f"Rejected: Elite Wallets {elite_n}/{MIN_ELITE_WALLETS}"
                    except Exception as e:
                        logger.debug("Elite-wallet gate check failed for %s: %s", mint, e)

                if gate_reason:
                    logger.info("%s — %s (%s…)", gate_reason, symbol, mint[:8])
                    _send_rejection_alert(mint, symbol, gate_reason.removeprefix("Rejected: "))
                    with activity_lock:
                        try:    del alerted_tokens[mint]
                        except KeyError: pass
                    should_buy_alert = False
            ...
            if should_buy_alert:
                cancel_reason = _check_buy_cancellation(mint, symbol, unique, first_detected_price)
                if cancel_reason:
                    logger.info("🚫 BUY SIGNAL cancelled — %s (%s…) — %s",
                               symbol, mint[:8], cancel_reason)
                    _send_rejection_alert(mint, symbol, cancel_reason)
                    with activity_lock:
                        try:    del alerted_tokens[mint]
                        except KeyError: pass
                    should_buy_alert = False
            ...
            if should_buy_alert:
                ...
                try:
                    ...
                    # sol_by_wallet sums each wallet's total SOL spent within the
                    # convergence window, seeding the per-wallet trade log used
                    # for the extended wallet quality metrics (hold time, rugs
                    # bought, avg entry mcap, realized PnL).
                    sol_by_wallet = {}
                    for (w, _t, s) in entries:
                        sol_by_wallet[w] = sol_by_wallet.get(w, 0) + (s or 0)
                    wi.start_alert_outcome_tracking(
                        mint, unique, price_data.get("price") or 0,
                        price_data.get("market_cap"), sol_by_wallet=sol_by_wallet)
                except Exception as e:
                    logger.debug("Intelligence alert hooks failed for %s: %s", mint, e)

                _record_alert_time()
                logger.info("🚨 BUY ALERT SENT — %s (%d wallets)", symbol, len(unique))
    ...
    sold = [t for t in transfers
            if t.get("fromUserAccount") == wallet and t.get("mint", "") not in IGNORED_MINTS]
    ...
    for transfer in sold:
        ...
        # ── Elite Exit Alert — fires the moment ANY Elite wallet sells a
        # token we already alerted on, regardless of whether that wallet was
        # part of the original triggering consensus group, and independent
        # of the normal take-profit/reduce/exit-now severity ladder below.
        # An Elite wallet exiting is significant on its own; it shouldn't
        # have to wait for enough OTHER wallets to also sell. Fires at most
        # once per token per trade cycle (elite_exit_alerted).
        if wallet in ELITE_WALLETS:
            with activity_lock:
                already_elite_exit = mint in elite_exit_alerted
                if not already_elite_exit:
                    elite_exit_alerted[mint] = ts
            if not already_elite_exit:
                try:
                    exit_price_data = _get_token_price(mint, bypass_cache=True)
                    exit_roi = (
                        (exit_price_data.get("price", 0) - entry_price) / entry_price * 100
                        if entry_price and exit_price_data.get("price") else None
                    )
                    roi_line = f"📉 ROI    : <b>{exit_roi:+.1f}%</b>\n" if exit_roi is not None else ""
                    _send_message_safe(
                        chat_id=CHAT_ID,
                        text=(
                            f"🔴⭐ <b>ELITE EXIT ALERT</b>\n\n"
                            f"🪙 Token   : <b>{_esc(symbol)}</b>\n"
                            f"👛 Wallet  : <code>{wallet[:4]}…{wallet[-4:]}</code>\n"
                            f"💰 Price   : <b>{_esc(exit_price_data.get('price_str', 'N/A'))}</b>\n"
                            f"{roi_line}"
                            f"⚠️ An Elite wallet is exiting this position\n\n"
                            f"📋 CA: <code>{_esc(mint)}</code>"
                        ),
                        parse_mode="HTML",
                    )
                except TelegramError as e:
                    logger.debug("Elite exit alert failed for %s: %s", mint, e)
                except Exception as e:
                    logger.debug("Elite exit alert build failed for %s: %s", mint, e)

        # Check if this wallet was one of the original buyers
        if wallet not in original_buyers:
            continue

        # NOTE: the standalone "first seller" early-warning alert that used to
        # fire here is now superseded by the Take Profit tier below, which
        # covers the same "at least one original buyer has sold" signal as
        # part of the proper severity ladder — keeping both would mean two
        # separate alerts for the same event. _alert_smart_money_exit() is
        # left in place (unused) in case a lighter-weight variant is wanted
        # again later.

        # Accumulate which original buyers are now selling
        with activity_lock:
            already_exit_alerted = mint in sell_alerted  # terminal exit_now already sent
            if not already_exit_alerted:
                ...
                # ── Exit severity — escalating tiers instead of one binary
                # "should I alert" decision. Take Profit / Reduce Position are
                # soft, informational nudges and fire regardless of hold time
                # (that's the whole point — replacing the jarring "buy alert
                # then immediate exit seconds later" behavior). Exit Now is
                # the terminal signal: it requires either ALL original buyers
                # to have sold (not just a majority — 2 of 3 selling is still
                # "reduce", not "consensus has exited"), or a fast coordinated
                # dump. Both bypass MIN_HOLD_TIME since either one is itself
                # the strongest possible signal regardless of timing. ───────
                all_sold = sell_total > 0 and sell_count >= sell_total
                is_exit_now = all_sold or fast_dump
                exit_now_ready = is_exit_now and (not within_hold_window or fast_dump or all_sold)
                ...
                level_rank    = {"take_profit": 1, "reduce": 2, "exit_now": 3}
                current_level = level_rank.get(severity, 0)
                try:    prior_level = sell_tier_alerted[mint]
                except Exception: prior_level = 0

                should_sell = current_level > 0 and current_level > prior_level
                if should_sell:
                    sell_tier_alerted[mint] = current_level
                    if severity == "exit_now":
                        sell_alerted[mint] = ts   # terminal — stop processing this mint
            else:
                should_sell = False
                severity    = "exit_now"
                sellers     = set()
        ...
        # Take Profit / Reduce Position are soft, informational nudges — the
        # trade cycle isn't actually over, so don't record a final outcome or
        # stop risk-monitoring this mint yet. Only Exit Now (the same-wallets-
        # have-left signal) is treated as the position being closed.
        if severity != "exit_now":
            continue
...
def helius_webhook():
    ...
    _diag_bump("webhook_calls", ts)
    with _diag_lock:
        _diag["txs_received"] += len(txs)
    for tx in txs:
...
@app.route("/roster-debug", methods=["GET"])
def roster_debug():
    """
    Compares three independent sources of truth for 'which wallets are we
    tracking', so a mismatch (like wallets that were removed from
    FALLBACK_WALLETS still showing up in alerts) can be pinned to exactly
    one of: the code, the local DB roster, or what's actually registered
    with Helius — instead of guessing.
    """
    code_wallets = set(FALLBACK_WALLETS)

    try:
        db_roster = set(wi.get_active_roster())
        db_ok = True
    except Exception as e:
        db_roster, db_ok = set(), False
        db_error = str(e)

    helius_wallets, helius_ok, helius_error = set(), False, None
    try:
        resp = requests.get(f"{HELIUS_BASE}/webhooks/{current_webhook_id}?api-key={HELIUS_API_KEY}",
                            timeout=10)
        resp.raise_for_status()
        helius_wallets = set(resp.json().get("accountAddresses", []))
        helius_ok = True
    except Exception as e:
        helius_error = str(e)

    # ── ALL webhooks on this API key, not just the one the bot thinks is
    # current — if there's more than one pointed at our URL, that's an
    # orphan still independently delivering events for whatever wallet list
    # it was created with, which would explain "old wallets" showing up even
    # after the dashboard/current webhook looks correct. ────────────────────
    all_webhooks_summary, all_webhooks_error = [], None
    try:
        resp = requests.get(f"{HELIUS_BASE}/webhooks?api-key={HELIUS_API_KEY}", timeout=10)
        resp.raise_for_status()
        our_url = f"{APP_URL.rstrip('/')}/helius-webhook"
        for h in (resp.json() or []):
            all_webhooks_summary.append({
                "webhookID":    h.get("webhookID"),
                "is_current":   h.get("webhookID") == current_webhook_id,
                "points_at_us": h.get("webhookURL") == our_url,
                "webhookURL":   h.get("webhookURL"),
                "wallet_count": len(h.get("accountAddresses") or []),
            })
    except Exception as e:
        all_webhooks_error = str(e)

    orphans = [h for h in all_webhooks_summary if h["points_at_us"] and not h["is_current"]]

    def _short(ws): return sorted(w[:8] + "…" for w in ws)

    return jsonify({
        "current_webhook_id": current_webhook_id,
        "counts": {
            "code_FALLBACK_WALLETS": len(code_wallets),
            "db_active_roster":      len(db_roster) if db_ok else "error",
            "helius_registered":     len(helius_wallets) if helius_ok else "error",
        },
        "code_vs_db": {
            "match": code_wallets == db_roster if db_ok else None,
            "in_code_not_db": _short(code_wallets - db_roster) if db_ok else None,
            "in_db_not_code": _short(db_roster - code_wallets) if db_ok else None,
        },
        "db_vs_helius": {
            "match": db_roster == helius_wallets if (db_ok and helius_ok) else None,
            "in_db_not_helius":     _short(db_roster - helius_wallets) if (db_ok and helius_ok) else None,
            "in_helius_not_db":     _short(helius_wallets - db_roster) if (db_ok and helius_ok) else None,
        },
        "all_webhooks_on_this_api_key": all_webhooks_summary,
        "orphan_webhooks_pointed_at_us": len(orphans),
        "orphan_webhook_ids": [h["webhookID"] for h in orphans],
        "helius_error": helius_error,
        "all_webhooks_error": all_webhooks_error,
    }), 200
...
def status():
    ...
    return jsonify({
        ...
        "diagnostics": {
            "webhook_calls_received": diag["webhook_calls"],
            "txs_received":           diag["txs_received"],
            "buys_observed":          diag["buys_observed"],
            "last_webhook_at":        diag["last_webhook_ts"],
            "last_buy_at":            diag["last_buy_ts"],
            "note": ("If webhook_calls_received is 0, Helius isn't reaching your "
                     "webhook URL (check ngrok/APP_URL). If it's >0 but buys_observed "
                     "stays 0, transactions are arriving but none involve your "
                     "watched wallets buying — that's normal, just wait longer."),
        },
    }), 200
...
def _bootstrap():
    ...
    logger.info("🚀 Starting DualChainWalletBot...")
