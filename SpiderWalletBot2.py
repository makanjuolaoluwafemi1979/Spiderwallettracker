...
def _process_tx(tx: dict, ts: int):
    ...
    if sig:
        ...
        if is_dup:
            dup_wallet = tx.get("feePayer", "").strip()
            dup_mint = ""
            for t in tx.get("tokenTransfers", []) or []:
                m = t.get("mint", "")
                if m and m != WSOL_MINT:
                    dup_mint = m
                    break
            # Cheap lookup only — no fresh symbol resolution (network call)
            # for a tx we're about to discard anyway.
            with _jupiter_map_lock:
                dup_symbol = _jupiter_token_map.get(dup_mint)
            if not dup_symbol:
                dup_symbol = (dup_mint[:8] + "…") if dup_mint else "unknown"
            logger.info(
                "🔁 Duplicate transaction skipped\n\nWallet: %s\nToken : %s\nSignature: %s",
                (dup_wallet[:8] + "…") if dup_wallet else "unknown",
                dup_symbol, (sig[:8] + "…") if sig else "unknown")
            return
    ...
    for transfer in bought:
        ...
        if should_buy_alert:
            ...
            if mcap > MAX_MCAP:
                ...
                logger.info(f"Skipping {symbol} — mcap ${mcap:,.0f} < MIN_MCAP")
                with activity_lock:
                    try:    del alerted_tokens[mint]
                    except KeyError: pass
                should_buy_alert = False
                ...
                if grade_data["final_score"] < MIN_GRADE_SCORE:
                    ...
                    try:
                        ...
                        if elite_n < MIN_ELITE_WALLETS:
                            gate_reason = f"{elite_n} elite wallet(s) < {MIN_ELITE_WALLETS} required"
                    except Exception as e:
